...
const FIELDS = "id,name,mimeType,parents,trashed,size,md5Checksum,modifiedTime,version,appProperties";

interface DriveFile { id: string; name?: string; mimeType?: string; parents?: string[]; trashed?: boolean; size?: string; md5Checksum?: string; modifiedTime?: string; version?: string; appProperties?: Record<string,string>; }
...
function evidence(file: DriveFile): ContentEvidence {
  return {
    ...(file.md5Checksum ? { hash: hash(`md5:${file.md5Checksum}`) } : {}),
...
export class GoogleDriveAdapter implements GoogleDrivePort {
  ...
  async authenticationState(): Promise<DriveAuthenticationState> {
    ...
    const data = await json<AboutResponse>(about.value); const key = data.user?.permissionId ?? data.user?.emailAddress;
  ...
  async createManagedRoot(vaultIdentity: VaultIdentity, protocolVersion: ProtocolVersion): Promise<DriveResult<ManagedRemoteIdentity>> {
    ...
    const metadata = { name: "BRAIN Sync", mimeType: FOLDER_MIME, appProperties: { [APP_ROLE]: ROOT_ROLE, [APP_VAULT]: String(vaultIdentity), [APP_PROTOCOL]: String(protocolVersion) } };
    const created = await this.metadataCreate(metadata); if (!created.ok) return created;
  ...
  async listForReconciliation(rootId: RemoteObjectId): Promise<DriveResult<RemoteListing>> {
    const guard = await this.guardPairedAccount(); if (!guard.ok) return guard;
    const validation = await this.validateRootExists(rootId); if (!validation.ok) return validation;
    const contentRoot = await this.contentRoot(rootId); if (!contentRoot.ok) return contentRoot;
    const entries: RemoteEntry[] = []; this.pathCache.clear();
    const queue: Array<{ id: string; path: string }> = [{ id: contentRoot.value.id, path: "" }];
    while (queue.length) {
      const current = queue.shift()!; let pageToken: string | undefined;
      do {
        const params = new URLSearchParams({ q: `'${escaped(current.id)}' in parents`, fields: `nextPageToken,files(${FIELDS})`, spaces: "drive", pageSize: "1000" });
        if (pageToken) params.set("pageToken", pageToken);
        const response = await this.transport.request(`${DRIVE_API}/files?${params}`);
        if (!response.ok) return entries.length ? { ok: true, value: { entries, completeness: { status: "partial", reason: response.signal.kind } } } : response;
        const page = await json<FileListResponse>(response.value);
        for (const file of page.files ?? []) {
          const path = joinPath(current.path, file.name ?? ""); const mapped = entry(path, file); entries.push(mapped); this.pathCache.set(file.id, path);
          if (file.mimeType === FOLDER_MIME) queue.push({ id: file.id, path: String(path) });
        }
  ...
  async readChanges(rootId: RemoteObjectId, changeCursor: ChangeCursor): Promise<DriveResult<RemoteChangePage>> {
    ...
    for (const change of page.changes ?? []) {
      if (change.fileId === String(rootId) || change.fileId === contentRoot.value.id) {
      ...
      if (change.removed || !change.file) {
        const lastKnownPath = this.pathCache.get(change.fileId); if (lastKnownPath) changes.push({ kind: "removed", remoteObjectId: rid(change.fileId), lastKnownPath });
        continue;
      }
      const path = await this.pathForFile(change.file, contentRoot.value.id);
      if (!path.ok) { if (path.signal.kind === "not-found") continue; return path; }
      if (!path.value) { if (this.pathCache.has(change.fileId)) return { ok: false, signal: { kind: "recovery-required", detail: "managed-object-left-remote-domain" } }; continue; }
      this.pathCache.set(change.fileId, path.value); changes.push({ kind: "upsert", entry: entry(path.value, change.file) });
    }
  ...
  async observe(rootId: RemoteObjectId, path: VaultPath): Promise<DriveResult<RemoteObservation>> {
    const guard = await this.guardPairedAccount(); if (!guard.ok) return guard;
    const resolved = await this.resolvePath(rootId, path); if (!resolved.ok) return resolved;
  ...
  async download(remoteObjectId: RemoteObjectId): Promise<DriveResult<RemoteDownload>> {
    ...
    const ev = evidence(meta.value); const size = ev.sizeBytes; const self = this;
    const content: BinaryContentSource = {
      ...
      async *openChunks() {
        ...
        while (size === undefined || offset < size) {
          ...
          const bytes = new Uint8Array(await result.value.arrayBuffer()); if (!bytes.length) break;
          yield bytes; offset += bytes.length;
  ...
  async create(rootId: RemoteObjectId, request: RemoteCreateRequest): Promise<DriveResult<RemoteMutationReceipt>> {
    const guard = await this.guardPairedAccount(); if (!guard.ok) return guard;
    const parent = await this.ensureParent(rootId, parentPath(request.path)); if (!parent.ok) return parent;
    if (request.entityKind === "folder") {
      const created = await this.metadataCreate({ name: segmentName(request.path), mimeType: FOLDER_MIME, parents: [parent.value] }); if (!created.ok) return created;
  ...
  async move(remoteObjectId: RemoteObjectId, _fromPath: VaultPath, toPath: VaultPath): Promise<DriveResult<RemoteMutationReceipt>> {
    ...
    const root = await this.findManagedRootAncestor(file.value); if (!root.ok) return root;
    if (!root.value) return { ok: false, signal: { kind: "recovery-required", detail: "move-object-outside-managed-remote" } };
    const parent = await this.ensureParent(rid(root.value), parentPath(toPath)); if (!parent.ok) return parent;
    ...
    const response = await this.transport.request(`${DRIVE_API}/files/${encodeURIComponent(String(remoteObjectId))}?${params}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: segmentName(toPath) }) });
  ...
  private async validateByExpected(rootId: RemoteObjectId, expected: VaultIdentity): Promise<DriveResult<ManagedRemoteValidation>> {
    ...
    const observedVault = file.value.appProperties?.[APP_VAULT]; const observedProtocol = file.value.appProperties?.[APP_PROTOCOL];
    ...
    const content = await this.contentRoot(rootId); if (!content.ok) return content.signal.kind === "recovery-required" ? { ok: true, value: { status: "ambiguous", reason: content.signal.detail } } : content;
  ...
  private async currentAccountKey(): Promise<DriveResult<string>> {
    ...
    const data = await json<AboutResponse>(response.value); const key = data.user?.permissionId ?? data.user?.emailAddress;
  ...
  private async children(parentId: string, name?: string): Promise<DriveResult<DriveFile[]>> {
    ...
    const params = new URLSearchParams({ q, fields: `files(${FIELDS})`, spaces: "drive", pageSize: "1000" });
    const response = await this.transport.request(`${DRIVE_API}/files?${params}`); if (!response.ok) return response;
    return { ok: true, value: (await json<FileListResponse>(response.value)).files ?? [] };
  }

  private async resolvePath(rootId: RemoteObjectId, path: VaultPath): Promise<DriveResult<DriveFile | undefined>> {
    const content = await this.contentRoot(rootId); if (!content.ok) return content; let parent = content.value;
    const segments = String(path).split("/").filter(Boolean); if (!segments.length) return { ok: true, value: content.value };
  ...
  private async ensureParent(rootId: RemoteObjectId, path: VaultPath): Promise<DriveResult<string>> {
    const content = await this.contentRoot(rootId); if (!content.ok) return content; let parent = content.value.id;
  ...
  private async pathForFile(file: DriveFile, contentRootId: string): Promise<DriveResult<VaultPath | undefined>> {
    const cached = this.pathCache.get(file.id); if (cached) return { ok: true, value: cached };
    ...
    while (true) {
      ...
      if (parentId === contentRootId) return { ok: true, value: vpath(names.reverse().join("/")) };
  ...
  private async findManagedRootAncestor(file: DriveFile): Promise<DriveResult<string | undefined>> {
    ...
    while (current.parents?.[0]) {
      ...
      const parent = await this.getFile(rid(parentId)); if (!parent.ok) return parent.signal.kind === "not-found" ? { ok: true, value: undefined } : parent;
      if (parent.value.appProperties?.[APP_ROLE] === ROOT_ROLE) return { ok: true, value: parent.value.id };
  ...
  private async resumableUpload(mode: "create"|"update", objectId: RemoteObjectId | undefined, path: VaultPath, parentId: string | undefined, content: BinaryContentSource, expected?: ContentEvidence): Promise<DriveResult<RemoteMutationReceipt>> {
    const metadata: Record<string, unknown> = { name: segmentName(path) }; if (mode === "create" && parentId) metadata.parents = [parentId];
    ...
    let offset = 0; let finalResponse: Response | undefined;
    for await (const part of rechunk(content, UPLOAD_CHUNK_BYTES)) {
      const end = offset + part.bytes.length - 1; const total = part.final ? String(offset + part.bytes.length) : "*";
      ...
      if (finalResponse.status === 308) { offset += part.bytes.length; continue; }
      offset += part.bytes.length; break;
    }
    if (content.sizeBytes === 0) {
      const sent = await this.transport.request(location, { method: "PUT", headers: { "content-length": "0", "content-range": "bytes */0" }, body: new Uint8Array() }); if (!sent.ok) return sent; finalResponse = sent.value;
    }
    ...
    const uploaded = await json<DriveFile>(finalResponse); const ev = evidence(uploaded);
    ...
    if (expected?.hash && String(expected.hash).startsWith("md5:") && ev.hash && String(expected.hash) !== String(ev.hash)) return { ok: false, signal: { kind: "recovery-required", detail: "uploaded-hash-integrity-mismatch" } };
  ...
  private async queryUploadOffset(location: string, total?: number): Promise<DriveResult<{ offset: number; completed: boolean; response?: Response }>> {
    ...
    const range = response.value.headers.get("range"); const end = range ? Number(range.split("-").at(-1)) : -1;
...
async function* rechunk(source: BinaryContentSource, size: number): AsyncIterable<{ bytes: Uint8Array; final: boolean }> {
  ...
  for await (const input of source.openChunks()) {
    ...
    const combined = new Uint8Array(pending.length + input.length); combined.set(pending); combined.set(input, pending.length); pending = combined;
