...
export const TRANSITIVELY_CARRIED_MOVE_REASON = "ancestor-folder-move-carried-descendant";
...
function exactTransitiveDestination(parent: PlannedOperation, child: PlannedOperation): boolean {
...
export class DeterministicSynchronizationPlanner implements SynchronizationPlanner {
  constructor(private readonly conflicts: ConflictResolver, private readonly destructiveSafety = new DestructiveSafetyPolicy(), private readonly options: PlannerOptions = {}) {}
  ...
  async plan(input: PlanningInput): Promise<SynchronizationPlan> {
    ...
    for (const snapshot of input.snapshots) {
      ...
      if (unsafe) { operations.push(blocked(snapshot.path, index++, unsafe.code, unsafe.summary, unsafe.recovery)); continue; }
  ...
  private classifyProvenMoves(snapshots: readonly PathSnapshot[], bases: readonly BaseEntry[], operations: PlannedOperation[], handled: Set<string>, nextIndex: () => number): void {
