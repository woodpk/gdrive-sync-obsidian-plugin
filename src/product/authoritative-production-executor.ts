...
type PreparedEffect = { readonly effect: RecoverableMutationEffectV1_1; readonly content?: BinaryContentSource; readonly localTransaction?: LocalMutationTransaction };
...
function comparisonKey(target: VaultPath): string { return String(target).replace(/\\/g, "/").normalize("NFC").toLocaleLowerCase("en-US"); }
...
function evidenceRef(prefix: string, value: unknown): string { return `${prefix}:${String(sha256Text(JSON.stringify(value)))}`; }
function success(operation: ExecutablePlannedOperation, resultingRemoteObjectId?: RemoteObjectId): ExecutionResult {
  const receipt: VerifiedExecutionReceipt = { operationId: operation.operationId, durable: true, integrityVerified: true, evidence: operation.contentVersion?.content, resultingRemoteObjectId, verificationEvidenceRef: `durable-effects:${String(operation.operationId)}` };
...
function resultReason(value: unknown, fallback: string): string {
  return typeof value === "object" && value !== null && "reason" in value && typeof (value as { reason?: unknown }).reason === "string" ? (value as { reason: string }).reason : fallback;
}
...
function localTransaction(operation: ExecutablePlannedOperation, intended: CanonicalFileContentProof, suffix: string, mutationKind: "create" | "replace"): LocalMutationTransaction | undefined {
  ...
  const common = { transactionId, operationId: operation.operationId, path: operation.path, stagePath: transactionPath(operation, "stage"), backupPath: transactionPath(operation, "backup"), stage: "staging" as const, expectedEntityKind: "file" as const, expectedNewEvidence: intended };
  ...
  return { ...common, mutationKind, expectedTarget: { status: "expected-present", observationToken: cid<"ObservationToken">(present.observationToken) as ObservationToken, entityKind: "file", canonicalContent: prior } };
}

async function prepareIntent(operation: ExecutablePlannedOperation, authority: SynchronizationAuthorityMetadataV1_1, legacy: ProductSynchronizationExecutor, deps: RecoverableProductionMutationDependencies, stateStore: SynchronizationStateStore, context: StateLoadContext, root: ManagedRemoteIdentity): Promise<{ status: "ready"; intent: RecoverableOperationIntentV1_1; prepared: PreparedEffect[] } | { status: "blocked"; reason: string }> {
  const intentId = intentIdFor(operation);
  const prepared: PreparedEffect[] = [];
  const add = (suffix: string, descriptor: RecoverablePhysicalMutationDescriptorV1_1, content?: BinaryContentSource, tx?: LocalMutationTransaction) => prepared.push({ effect: { effectId: effectIdFor(operation, suffix), descriptor, stage: "intent-persisted" }, content, localTransaction: tx });
  ...
  if (operation.kind === "upload-create") {
    ...
    if (version.entityKind === "folder") {
      ...
      add("remote-folder", { kind: "remote-folder-create", targetSide: "remote", mutationKind: "create", intentId, targetPath: operation.path, parentRemoteObjectId, pathAuthority: { generation: authority.semanticGeneration, targetPath: operation.path, parentPath: parentPath(operation.path), pathComparisonKey: comparisonKey(operation.path), expectedTarget: "absent" }, remoteMutation: reserved.value });
    } else {
      const source = await localOrRetainedSource(legacy, version); const intended = canonical(source?.evidence ?? version.content);
    ...
    const source = await localOrRetainedSource(legacy, operation.contentVersion); const intended = canonical(source?.evidence ?? operation.contentVersion.content); const revision = expectedRemoteRevision(operation, operation.remoteObjectId);
    ...
    add("remote-file", { kind: "remote-file", targetSide: "remote", mutationKind: "update", targetPath: operation.path, remoteMutation: { kind: "existing-file-content-update", intentId, remoteObjectId: operation.remoteObjectId, expectedRevision: revision, path: operation.path, updateProtocol: "immutable-candidate-preservation", candidateRemoteObjectId: candidate.value.reservedRemoteObjectId, intendedContent: intended, identityAuthority: identity }, intendedContent: intended }, source.content);
  } else if (operation.kind === "download-create" || operation.kind === "download-update") {
    const version = operation.contentVersion; const remoteObjectId = version?.remoteObjectId ?? operation.remoteObjectId;
    if (!version || version.entityKind !== "file" || !remoteObjectId) return { status: "blocked", reason: "LOCAL file mutation lacks exact REMOTE version" };
    const source = await remoteSource(legacy, version, remoteObjectId); const intended = canonical(source?.evidence ?? version.content);
    if (!source || !intended) return { status: "blocked", reason: "LOCAL file mutation requires canonical downloaded evidence" };
    const kind = operation.kind === "download-create" ? "create" as const : "replace" as const; const tx = localTransaction(operation, intended, "file", kind);
    ...
    add(operation.targetSide === "remote" ? "remote-move" : "local-move", { kind: "move", targetSide: operation.targetSide ?? "local", fromPath: operation.fromPath, toPath: operation.toPath, remoteObjectId: operation.remoteObjectId, identityAuthority: identity });
    ...
    add(operation.kind === "trash-remote" ? "remote-trash" : "local-trash", { kind: "trash", targetSide: operation.kind === "trash-remote" ? "remote" : "local", path: operation.path, remoteObjectId: operation.remoteObjectId, baseAuthority: base, identityAuthority: identity });
    ...
    const source = await localOrRetainedSource(legacy, version); const intended = canonical(version.content); const revision = expectedRemoteRevision(operation, operation.remoteObjectId);
    if (!source || !intended || !revision) return { status: "blocked", reason: "clean merge lacks canonical retained bytes or REMOTE revision" };
    const tx = localTransaction(operation, intended, "merge", "replace"); if (!tx) return { status: "blocked", reason: "clean merge LOCAL side lacks exact prior authority" };
    ...
    const candidate = await remote.reserveFileCreateIdentity(root, intentId, operation.path, intended); if (!candidate.ok) return { status: "blocked", reason: `clean merge candidate reservation failed: ${candidate.signal.kind}` };
...
async function verifyRemote(legacy: ProductSynchronizationExecutor, descriptor: RecoverablePhysicalMutationDescriptorV1_1): Promise<Convergence> {
  const reads = internal(legacy); const result = await reads.drive.listForReconciliation(reads.runEvidence().managedRemote.rootId);
  ...
  if (descriptor.kind === "trash" && descriptor.targetSide === "remote") return active.some(value => value.remoteObjectId === descriptor.remoteObjectId) ? { ok: false, reason: "REMOTE trash target remains present" } : { ok: true };
  return { ok: true };
}
async function verifyLocal(legacy: ProductSynchronizationExecutor, descriptor: RecoverablePhysicalMutationDescriptorV1_1): Promise<Convergence> {
  ...
  if (descriptor.kind === "local-file") {
    ...
    return observed.status === "present" && observed.entityKind === "file" && observed.content?.hash === descriptor.intendedContent.hash && observed.content.sizeBytes === descriptor.intendedContent.sizeBytes ? { ok: true } : { ok: false, reason: "LOCAL file exact intended content is not observable" };
  }
  if (descriptor.kind === "local-folder-create") {
    ...
    const folderObservation: LocalFolderCreateObservation = observed.status === "present" && observed.entityKind === "folder" && observed.observationToken
      ...
      : observed.status === "absent" ? { status: "authoritative-absent", targetPath: descriptor.targetPath, pathComparisonKey: descriptor.pathAuthority.pathComparisonKey }
        : observed.status === "present" ? { status: "occupied", targetPath: descriptor.targetPath, pathComparisonKey: descriptor.pathAuthority.pathComparisonKey, entityKind: observed.entityKind }
          : { status: "unobservable", reason: "LOCAL folder observation incomplete" };
    const result = verifyLocalFolderCreate(descriptor, folderObservation); return result.status === "verified-effect" ? { ok: true } : { ok: false, reason: result.reason };
  }
  if (descriptor.kind === "move" && descriptor.targetSide === "local") { const [from, to] = await Promise.all([local.observe(descriptor.fromPath), local.observe(descriptor.toPath)]); return from.status === "absent" && to.status === "present" ? { ok: true } : { ok: false, reason: "LOCAL move is not converged" }; }
  if (descriptor.kind === "trash" && descriptor.targetSide === "local") return (await local.observe(descriptor.path)).status === "absent" ? { ok: true } : { ok: false, reason: "LOCAL trash target remains present" };
  return { ok: true };
}
async function convergenceFor(legacy: ProductSynchronizationExecutor, descriptor: RecoverablePhysicalMutationDescriptorV1_1): Promise<Convergence> { return descriptor.targetSide === "remote" ? verifyRemote(legacy, descriptor) : verifyLocal(legacy, descriptor); }

async function recoverEffect(lifecycle: DurableEffectLifecycleCoordinator, operation: ExecutablePlannedOperation, effect: RecoverableMutationEffectV1_1, legacy: ProductSynchronizationExecutor, deps: RecoverableProductionMutationDependencies): Promise<ExecutionResult | undefined> {
  ...
  if (effect.stage === "dispatch-authorized" || effect.stage === "outcome-unknown") {
    ...
    if (effect.descriptor.kind === "remote-folder-create") {
      ...
      const observation = await deps.remoteFolderCreateRecoveryReadPort.observeFolderCreateRecovery(effect.descriptor); const verified = verifyRemoteFolderCreate(effect.descriptor, observation);
      ...
      const loaded = await lifecycle.loadAuthority(); if (loaded.status !== "trusted") return { status: "recovery-required", reason: loaded.reason };
      const localDescriptor = effect.descriptor;
      const tx = loaded.state.localTransactions.find(value => value.transactionId === localDescriptor.localTransactionId); if (!tx) return { status: "recovery-required", reason: "LOCAL recovery transaction missing" };
      const recovered = await deps.localTransactionalMutationPort.recover(tx); const persisted = await lifecycle.persistLocalTransaction(recovered.transaction);
      ...
      physical = recovered.status === "recovered" && converged.ok ? { status: "verified-effect", verificationEvidenceRef: evidenceRef("local-file-recovery", String(tx.transactionId)) } : { status: "outcome-unknown", reason: recovered.status === "recovered" ? (converged.ok ? "LOCAL recovery convergence unavailable" : converged.reason) : resultReason(recovered, "LOCAL recovery unresolved") };
    } else {
      const converged = await convergenceFor(legacy, effect.descriptor); physical = converged.ok ? { status: "verified-effect", verificationEvidenceRef: evidenceRef("restart-observation", effect.effectId) } : { status: "outcome-unknown", reason: converged.reason };
    }
    const recorded = await lifecycle.recordPhysicalResult(String(operation.operationId), effect.effectId, physical);
    if (recorded.status !== "effect-verified" && recorded.status !== "already-progressed") return recorded.status === "conflict-preserved" ? { status: "blocking-failure", reason: recorded.reason } : { status: "uncertain", reason: resultReason(recorded, "physical recovery unresolved") };
  }
  const loaded = await lifecycle.loadAuthority(); if (loaded.status !== "trusted") return { status: "recovery-required", reason: loaded.reason };
  ...
  const converged = await convergenceFor(legacy, current.descriptor); if (!converged.ok) return { status: "blocking-failure", reason: converged.reason };
...
async function dispatchEffect(lifecycle: DurableEffectLifecycleCoordinator, operation: ExecutablePlannedOperation, prepared: PreparedEffect, legacy: ProductSynchronizationExecutor, deps: RecoverableProductionMutationDependencies): Promise<ExecutionResult | undefined> {
  ...
  const descriptor = prepared.effect.descriptor; let physical: PhysicalEffectDispatchResult;
  if (descriptor.kind === "remote-file") {
    ...
    const outcome = descriptor.mutationKind === "create" ? await deps.reliableRemoteMutationPort.createReserved(descriptor.remoteMutation as Extract<typeof descriptor.remoteMutation, { kind: "reserved-file-create" }>, prepared.content) : await deps.reliableRemoteMutationPort.updateExisting(descriptor.remoteMutation as Extract<typeof descriptor.remoteMutation, { kind: "existing-file-content-update" }>, prepared.content); physical = mapRemoteOutcome(outcome);
    ...
    await deps.reliableRemoteMutationPort.createReserved(descriptor.remoteMutation); const observed = await deps.remoteFolderCreateRecoveryReadPort.observeFolderCreateRecovery(descriptor); const verified = verifyRemoteFolderCreate(descriptor, observed); physical = verified.status === "verified-effect" ? { status: "verified-effect", verificationEvidenceRef: evidenceRef("remote-folder", verified.proof) } : verified;
    ...
    physical = mapRemoteOutcome(await deps.reliableRemoteMutationPort.moveExisting({ kind: "identity-preserving-move", intentId: intentIdFor(operation), remoteObjectId: descriptor.remoteObjectId, fromPath: descriptor.fromPath, toPath: descriptor.toPath, identityAuthority: descriptor.identityAuthority }));
    ...
    physical = mapRemoteOutcome(await deps.reliableRemoteMutationPort.trashExisting({ kind: "trash", intentId: intentIdFor(operation), remoteObjectId: descriptor.remoteObjectId, path: descriptor.path, baseAuthority: descriptor.baseAuthority, identityAuthority: descriptor.identityAuthority }));
    ...
    const staged = await deps.localTransactionalMutationPort.stageAndVerify(prepared.localTransaction, prepared.content); let saved = await lifecycle.persistLocalTransaction(staged.transaction); if (saved.status !== "persisted") return { status: "recovery-required", reason: "LOCAL staged progress not durable" };
    if (staged.status !== "staged-verified") physical = { status: "outcome-unknown", reason: resultReason(staged, "LOCAL stage unresolved") };
    else { const committed = await deps.localTransactionalMutationPort.commitVerifiedStage(staged.transaction); saved = await lifecycle.persistLocalTransaction(committed.transaction); if (saved.status !== "persisted") return { status: "recovery-required", reason: "LOCAL commit progress not durable" }; const converged = await verifyLocal(legacy, descriptor); physical = committed.status === "committed" && converged.ok ? { status: "verified-effect", verificationEvidenceRef: evidenceRef("local-file", String(committed.transaction.transactionId)) } : { status: "outcome-unknown", reason: committed.status === "committed" ? (converged.ok ? "LOCAL convergence unavailable" : converged.reason) : resultReason(committed, "LOCAL transaction unresolved") }; }
  } else if (descriptor.kind === "local-folder-create") {
    await internal(legacy).local.createFolder(descriptor.targetPath); const converged = await verifyLocal(legacy, descriptor); physical = converged.ok ? { status: "verified-effect", verificationEvidenceRef: evidenceRef("local-folder", String(descriptor.targetPath)) } : { status: "outcome-unknown", reason: converged.reason };
  } else if (descriptor.kind === "move" && descriptor.targetSide === "local") {
    await internal(legacy).local.move(descriptor.fromPath, descriptor.toPath); const converged = await verifyLocal(legacy, descriptor); physical = converged.ok ? { status: "verified-effect", verificationEvidenceRef: evidenceRef("local-move", [String(descriptor.fromPath), String(descriptor.toPath)]) } : { status: "outcome-unknown", reason: converged.reason };
  } else if (descriptor.kind === "trash" && descriptor.targetSide === "local") {
    await internal(legacy).local.trash(descriptor.path); const converged = await verifyLocal(legacy, descriptor); physical = converged.ok ? { status: "verified-effect", verificationEvidenceRef: evidenceRef("local-trash", String(descriptor.path)) } : { status: "outcome-unknown", reason: converged.reason };
  } else return { status: "recovery-required", reason: "unsupported durable descriptor" };

  const recorded = await lifecycle.recordPhysicalResult(String(operation.operationId), prepared.effect.effectId, physical);
  if (recorded.status !== "effect-verified") return recorded.status === "conflict-preserved" ? { status: "blocking-failure", reason: recorded.reason } : { status: "uncertain", reason: resultReason(recorded, `physical effect remained ${recorded.status}`) };
  const converged = await convergenceFor(legacy, descriptor); if (!converged.ok) return { status: "blocking-failure", reason: converged.reason };
  const current = recorded.authority.operationIntents.find(value => value.operationId === operation.operationId)?.effects.find(value => value.effectId === prepared.effect.effectId); if (!current?.verificationEvidenceRef) return { status: "recovery-required", reason: "durable verification evidence missing" };
  const committed = await lifecycle.markEffectStateCommitted(String(operation.operationId), current.effectId, current.verificationEvidenceRef); return committed.status === "state-committed" ? undefined : { status: "recovery-required", reason: "effect could not reach state-committed" };
}

export function createAuthoritativeProductExecutor(legacy: ProductSynchronizationExecutor, authorityStore: SynchronizationAuthorityStoreV1_1, identityStateStore: SynchronizationStateStore, stateContext: StateLoadContext, managedRemote: ManagedRemoteIdentity, explicitDependencies?: RecoverableProductionMutationDependencies): AuthoritativeSynchronizationExecutor {
  ...
  async function validateExact(operation: ExecutablePlannedOperation): Promise<AuthorityCompletePreconditionValidationResult> {
    ...
    for (const precondition of operation.preconditions) {
      if (precondition.kind === "base-authority") {
        ...
        const proof = precondition.proof; const currentPath = authorityLoad.state.pathConvergence.find(value => value.path === proof.path)?.state; const byPath = identityLoad.state.remoteMappings.filter(value => value.path === proof.path); const byId = identityLoad.state.remoteMappings.filter(value => value.remoteObjectId === proof.remoteObjectId); const mapping = byPath.length === 1 && byId.length === 1 && byPath[0] === byId[0] ? byPath[0] : undefined;
        if (!mapping || !currentPath || currentPath.status !== "converged" || currentPath.generation !== authorityLoad.state.semanticGeneration || proof.generation !== authorityLoad.state.semanticGeneration) failed.push(precondition);
        else { const observedPath = operation.kind === "identity-preserving-move" && operation.targetSide === "local" && operation.toPath ? operation.toPath : mapping.path; if (!await legacy.versionStillCurrent("remote", { path: observedPath, entityKind: mapping.entityKind, remoteObjectId: mapping.remoteObjectId }, managedRemote)) failed.push(precondition); }
      }
    }
    if (failed.length) return { status: "stale", failed };
    const ordinary = await legacy.validatePreconditions(operation); if (ordinary.status === "valid") return { status: "valid" }; if (ordinary.status === "stale") return { status: "stale", failed: ordinary.failed.filter((value): value is ExecutableOperationPrecondition => value.kind !== "base-trusted" && value.kind !== "identity-unambiguous") }; return ordinary;
  }
  return { validatePreconditions: validateExact, async execute(operation) {
    const validation = await validateExact(operation); if (validation.status === "stale") return { status: "stale-precondition", reason: "exact authority changed at production mutation boundary", failed: validation.failed }; if (validation.status === "blocked") return { status: "blocking-failure", reason: validation.reason }; if (validation.status === "recovery-required") return { status: "recovery-required", reason: validation.reason }; if (!physicalOperation(operation)) return legacy.execute(operation);
    const needsRemote = operation.targetSide === "remote" || operation.kind.startsWith("upload-") || operation.kind === "trash-remote" || operation.kind === "clean-text-merge"; const needsLocalFile = operation.kind === "download-create" || operation.kind === "download-update" || operation.kind === "clean-text-merge";
    if (needsRemote && !dependencies.reliableRemoteMutationPort) return { status: "recovery-required", reason: "ReliableRemoteMutationPort unavailable; physical mutation disabled" }; if (needsLocalFile && !dependencies.localTransactionalMutationPort) return { status: "recovery-required", reason: "LocalTransactionalMutationPort unavailable; physical mutation disabled" }; if (operation.kind === "upload-create" && operation.contentVersion?.entityKind === "folder" && !dependencies.remoteFolderCreateRecoveryReadPort) return { status: "recovery-required", reason: "RemoteFolderCreateRecoveryReadPort unavailable; REMOTE folder mutation disabled" };
    const lifecycle = new DurableEffectLifecycleCoordinator(authorityStore); const loaded = await lifecycle.loadAuthority(); if (loaded.status !== "trusted") return { status: "recovery-required", reason: loaded.reason };
    const existing = loaded.state.operationIntents.find(value => value.operationId === operation.operationId);
    if (existing) { if (existing.semanticAuthority.generation !== loaded.state.semanticGeneration) return { status: "recovery-required", reason: "persisted intent belongs to stale semantic authority" }; for (const effect of existing.effects) { const result = await recoverEffect(lifecycle, operation, effect, legacy, dependencies); if (result) return result; } const final = await lifecycle.loadAuthority(); if (final.status !== "trusted") return { status: "recovery-required", reason: final.reason }; const complete = final.state.operationIntents.find(value => value.operationId === operation.operationId); if (!complete?.effects.every(value => value.stage === "state-committed")) return { status: "recovery-required", reason: "restart did not complete every required effect" }; const remoteDescriptor = complete.effects.map(value => value.descriptor).find(value => value.kind === "remote-file"); const resulting = remoteDescriptor?.kind === "remote-file" ? (remoteDescriptor.remoteMutation.kind === "reserved-file-create" ? remoteDescriptor.remoteMutation.reservedRemoteObjectId : remoteDescriptor.remoteMutation.candidateRemoteObjectId) : operation.remoteObjectId; return success(operation, resulting); }
    const prepared = await prepareIntent(operation, loaded.state, legacy, dependencies, identityStateStore, stateContext, managedRemote); if (prepared.status === "blocked") return { status: "recovery-required", reason: prepared.reason }; const persisted = await lifecycle.persistIntent(prepared.intent, prepared.prepared.flatMap(value => value.localTransaction ? [value.localTransaction] : [])); if (persisted.status !== "persisted") return { status: "recovery-required", reason: `physical intent not durably persisted (${persisted.status})` }; for (const effect of prepared.prepared) { const result = await dispatchEffect(lifecycle, operation, effect, legacy, dependencies); if (result) return result; } const final = await lifecycle.loadAuthority(); if (final.status !== "trusted") return { status: "recovery-required", reason: final.reason }; const complete = final.state.operationIntents.find(value => value.operationId === operation.operationId); if (!complete?.effects.every(value => value.stage === "state-committed")) return { status: "recovery-required", reason: "logical operation incomplete: required effect not state-committed" }; const remoteDescriptor = complete.effects.map(value => value.descriptor).find(value => value.kind === "remote-file"); const resulting = remoteDescriptor?.kind === "remote-file" ? (remoteDescriptor.remoteMutation.kind === "reserved-file-create" ? remoteDescriptor.remoteMutation.reservedRemoteObjectId : remoteDescriptor.remoteMutation.candidateRemoteObjectId) : operation.remoteObjectId; return success(operation, resulting);
  } };
}
