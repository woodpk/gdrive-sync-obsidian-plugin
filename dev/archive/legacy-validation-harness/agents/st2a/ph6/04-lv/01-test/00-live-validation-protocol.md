...
- **Parallel Wave D** is explicitly authorized: VH24/D01, VH25/D02, VH26/D03, VH27/D04, VH28/D05, and VH29/D06 may execute concurrently.
- Before Parallel Wave D is dispatched, this D-series tasking refresh MUST be supervisor-approved and promoted to `phase6-integration`.
- Immediately before dispatch, `phase6-integration` is the frozen D-series task authority/base. After `git fetch origin --prune --tags`, the exact resolved `origin/phase6-integration` HEAD becomes `D_SERIES_COMMON_BASE_SHA` for the entire wave. The task prompts require no manual SHA substitution.
- Each Parallel Wave D task must independently resolve and record that same frozen `D_SERIES_COMMON_BASE_SHA`, create its own scenario branch from exactly that commit, own only its assigned scenario implementation/tests/evidence, consume no other D-series scenario branch, and preserve all frozen/shared semantic authority.
- All six VH24–VH29 task evidence records MUST identify the same `D_SERIES_COMMON_BASE_SHA`. Any mismatch is a hard stop and the D-series outputs MUST NOT be integrated.
- `phase6-integration` MUST NOT be advanced with D-series implementation work after the wave base is frozen and before Parallel Wave D has completed and VH30 integration is ready.
- If a scenario requires a shared contract change rather than implementation behind existing contracts, that task must stop and report the required contract-change condition instead of modifying the shared contract independently.
- VH30/H8I is **not** a member of Parallel Wave D. It remains the serial D-series integration gate and may begin only after VH24–VH29 have all independently completed and the supervisor has approved each exact scenario HEAD. VH30 must consume the same recorded `D_SERIES_COMMON_BASE_SHA` plus those six supervisor-approved exact scenario HEADs.
