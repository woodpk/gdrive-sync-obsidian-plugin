...
- either approve the complete build or issue deterministic, agent-targeted corrective work orders under `dev/prompts/rejection-fix-prompt-spec.md`.
...
2. read `dev/prompts/rejection-fix-prompt-spec.md` completely before creating coding-agent work orders;
...
3. one clearly separated, independently copyable corrective prompt per listed agent, each conforming to `dev/prompts/rejection-fix-prompt-spec.md`.
...
If `dev/prompts/rejection-fix-prompt-spec.md` is missing or unreadable, reject and output only the smallest safe notice identifying that missing required rejection authority.
