# VH29 D06 stale-device scenario verifier.
...
$ExpectedBranch = 'phase6-vh29-d06-scenario'
$ExpectedIntegrationSha = 'c6daa20ad287f395a99cf88943465a9ecc3159dd'
$FocusedTestCommand = 'node ./node_modules/typescript/bin/tsc -p tsconfig.test.json && node --test .test-build/test/validation-d06-stale-device.test.js .test-build/test/phase2-planner.test.js'
...
if ($integrationHead -cne $ExpectedIntegrationSha) {
    throw "D-SERIES COMMON BASE MISMATCH before verification. Expected $ExpectedIntegrationSha; origin/phase6-integration is $integrationHead."
}
...
Invoke-GitCheck -Label 'VH29 D06 committed diff check' -Arguments @('diff','--check',"$BaseSha..$remoteHead")
...
Write-Host "Frozen integration SHA: $integrationHead"
...
$postFetchOutput = @(& git -C $script:RepoRoot fetch origin --prune --tags 2>&1)
$postFetchExit = $LASTEXITCODE
foreach ($line in $postFetchOutput) { Write-Host ([string]$line) }
if ($postFetchExit -ne 0) { throw "post-verification git fetch failed with exit code $postFetchExit." }
$postIntegrationHead = Invoke-GitText @('rev-parse','refs/remotes/origin/phase6-integration')
if ($postIntegrationHead -cne $ExpectedIntegrationSha) {
    throw "D-SERIES COMMON BASE MISMATCH after verification. Expected $ExpectedIntegrationSha; origin/phase6-integration is $postIntegrationHead."
}

Write-Host ""
Write-Host "VH29 D06 VERIFICATION: PASS"
Write-Host "Change-set verification: PASS"
Write-Host "Repository verification: PASS"
Write-Host "Overall verification: PASS"
Write-Host "Frozen integration verification: PASS ($postIntegrationHead)"
