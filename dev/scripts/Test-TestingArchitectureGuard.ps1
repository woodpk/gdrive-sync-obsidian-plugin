...
function Get-BoundaryPolicy {
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($status)) {
            throw 'Manifest status is empty.'
        }
        ...
        if ($scenarioSpecificProductionAllowed -or $productionValidationUiAllowed) {
            throw 'Manifest permits production scenario/validation authority prohibited by S03B.'
        }
...
const buildKeys = new Set(["entry", "entries", "entryPoint", "entryPoints", "input", "inputs", "files", "include", "inject", "tsconfig"]);
function collectBuildReferences(sourceFile) {
  const topLevelValues = new Map();
  for (const statement of sourceFile.statements) {
    if (!ts.isVariableStatement(statement)) continue;
    for (const declaration of statement.declarationList.declarations) {
      if (ts.isIdentifier(declaration.name) && declaration.initializer) topLevelValues.set(declaration.name.text, declaration.initializer);
    }
  }
  ...
  const references = [];
  function addValues(node) { for (const value of staticStrings(node, new Set())) references.push(value); }
  function visit(node) {
    if (ts.isVariableDeclaration(node) && ts.isIdentifier(node.name) && buildKeys.has(node.name.text)) addValues(node.initializer);
    else if (ts.isPropertyAssignment(node)) {
      const key = propertyNameText(node.name);
      if (key && buildKeys.has(key)) addValues(node.initializer);
    } else if (ts.isShorthandPropertyAssignment(node) && buildKeys.has(node.name.text) && topLevelValues.has(node.name.text)) {
      addValues(topLevelValues.get(node.name.text));
    }
    ts.forEachChild(node, visit);
  }
  visit(sourceFile);
