# VH24 D01 clean-text-merge scenario verifier.
...
if ($integrationHead -cne $BaseSha) {
    throw "D-SERIES COMMON BASE MISMATCH before verification. Required $BaseSha; origin/phase6-integration is $integrationHead."
}
...
Invoke-GitCheck -Label 'VH24 D01 committed diff check' -Arguments @('diff','--check',"$BaseSha..$remoteHead")
...
if ($postIntegrationHead -cne $BaseSha) {
    throw "D-SERIES COMMON BASE MISMATCH after verification. Required $BaseSha; origin/phase6-integration is $postIntegrationHead."
}
