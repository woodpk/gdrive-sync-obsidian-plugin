...
var GoogleOAuthSession = class _GoogleOAuthSession {
  ...
  clearTokens() {
    ...
    this.accessFailure = "authentication-required";
  }
  accessTokenFailure() {
  ...
  invalidateAccessToken() {
    ...
    if (!current) return;
    if (!hasExactRequiredDriveScope(current.scope) || !current.refreshToken) {
      this.clearTokens();
      return;
    }
    this.secrets.set(_GoogleOAuthSession.TOKEN_SECRET_ID, encodeTokens({ ...current, accessToken: "", expiresAtMs: 0 }));
    this.accessFailure = void 0;
  }
  async accessToken() {
    const current = this.tokens();
    ...
    if (current.expiresAtMs - this.now() > 6e4 && current.accessToken) {
      this.accessFailure = void 0;
...
var TRANSITIVELY_CARRIED_MOVE_REASON = "ancestor-folder-move-carried-descendant";
...
function sameAsBase(version, base) {
...
function normalizedPath(value2) {
  return value2 === void 0 ? void 0 : String(value2).replace(/\\/g, "/").replace(/^\/+|\/+$/g, "");
}
function strictAncestor(parent, child) {
  return parent.length < child.length && child.startsWith(`${parent}/`);
}
function exactTransitiveDestination(parent, child) {
  const parentFrom = normalizedPath(parent.fromPath);
  const parentTo = normalizedPath(parent.toPath);
...
function orderNestedMoves(operations) {
...
function coalesceTransitivelyCarriedMoves(operations, state) {
  if (state.status !== "trusted") return [...operations];
  const baseByPath = new Map(state.state.base.map((entry2) => [String(entry2.path), entry2]));
  ...
  return ordered.map((operation, index) => {
    ...
    const ancestor2 = ordered.slice(0, index).find((candidate) => {
...
var DeterministicSynchronizationPlanner = class {
  ...
  finish(state, totalManagedPaths, operations) {
    const orderedOperations = coalesceTransitivelyCarriedMoves(operations, state);
    ...
    return { planId: semanticPlanId({ trigger, operations: orderedOperations, executionDisposition, recoveryCheckpointRequired, globalExecutionGate: globalExecutionGate2 }), trigger, operations: orderedOperations, executionDisposition, recoveryCheckpointRequired, globalExecutionGate: globalExecutionGate2 };
  }
};
...
function automaticReplaySafe(init) {
  ...
  request(url, init = {}, retry = true) {
    return this.semaphore.run(async () => {
      ...
      for (let attempt = 0; attempt < this.policy.maxAttempts; attempt++) {
        ...
        if (!token) {
          return this.oauth.accessTokenFailure() === "transient" ? { ok: false, signal: { kind: "transient-failure", detail: "oauth-refresh-deferred" } } : { ok: false, signal: { kind: "authentication-required", detail: "missing-or-expired-token" } };
        }
        let response;
        try {
...
function isNonEmptyPath(value2) {
  return isString(value2);
}
...
function isCanonicalContentProof(value2) {
  return isRecord(value2) && value2.algorithm === "sha256" && isString(value2.hash) && Number.isSafeInteger(value2.sizeBytes) && Number(value2.sizeBytes) >= 0;
}
function isIdentityAuthority(value2) {
  return isRecord(value2) && value2.status === "unique" && isString(value2.generation) && isNonEmptyPath(value2.path) && isString(value2.remoteObjectId);
}
function isBaseAuthority(value2) {
  return isRecord(value2) && isString(value2.generation) && isNonEmptyPath(value2.path) && isString(value2.fingerprint);
}
function canonicalContentEqual(a, b) {
  return isRecord(a) && isRecord(b) && a.algorithm === b.algorithm && a.hash === b.hash && a.sizeBytes === b.sizeBytes;
}
...
function isRemoteFileMutationShape(value2) {
  if (!isRecord(value2) || !isString(value2.intentId) || !isNonEmptyPath(value2.path)) return false;
  if (value2.kind === "reserved-file-create") {
    return isString(value2.reservedRemoteObjectId) && isCanonicalContentProof(value2.intendedContent);
  }
  if (value2.kind === "existing-file-content-update") {
    return isString(value2.remoteObjectId) && isString(value2.expectedRevision) && value2.updateProtocol === "immutable-candidate-preservation" && isString(value2.candidateRemoteObjectId) && isCanonicalContentProof(value2.intendedContent) && isIdentityAuthority(value2.identityAuthority);
  }
  return false;
}
function isV1DescriptorShape(descriptor) {
  ...
  switch (descriptor.kind) {
    case "local-file":
      return descriptor.targetSide === "local" && (descriptor.mutationKind === "create" || descriptor.mutationKind === "replace") && isNonEmptyPath(descriptor.targetPath) && isString(descriptor.localTransactionId) && isCanonicalContentProof(descriptor.intendedContent);
    case "remote-file":
      return descriptor.targetSide === "remote" && (descriptor.mutationKind === "create" || descriptor.mutationKind === "update") && isNonEmptyPath(descriptor.targetPath) && isRemoteFileMutationShape(descriptor.remoteMutation) && isCanonicalContentProof(descriptor.intendedContent);
    case "move":
      return (descriptor.targetSide === "local" || descriptor.targetSide === "remote") && isNonEmptyPath(descriptor.fromPath) && isNonEmptyPath(descriptor.toPath) && descriptor.fromPath !== descriptor.toPath && (descriptor.remoteObjectId === void 0 || isString(descriptor.remoteObjectId)) && isIdentityAuthority(descriptor.identityAuthority) && (descriptor.targetSide !== "remote" || isString(descriptor.remoteObjectId));
    case "trash":
      return (descriptor.targetSide === "local" || descriptor.targetSide === "remote") && isNonEmptyPath(descriptor.path) && (descriptor.remoteObjectId === void 0 || isString(descriptor.remoteObjectId)) && isBaseAuthority(descriptor.baseAuthority) && (descriptor.identityAuthority === void 0 || isIdentityAuthority(descriptor.identityAuthority)) && (descriptor.targetSide !== "remote" || isString(descriptor.remoteObjectId) && isIdentityAuthority(descriptor.identityAuthority));
    default:
      return false;
  }
}
...
function physicalDescriptorIssues(intent, effect) {
  const descriptor = effect.descriptor;
  if (descriptor.kind === "local-folder-create" || descriptor.kind === "remote-folder-create") return [];
  const issues = [];
  ...
  if (descriptor.kind === "move") {
    const identity2 = descriptor.identityAuthority;
    if (identity2.generation !== generation || identity2.path !== descriptor.fromPath || descriptor.remoteObjectId !== void 0 && descriptor.remoteObjectId !== identity2.remoteObjectId || descriptor.targetSide === "remote" && descriptor.remoteObjectId !== identity2.remoteObjectId) issues.push(issue("other-semantic-inconsistency", "move descriptor identity/from-path authority is inconsistent", descriptor.fromPath, "journal-descriptor"));
    return issues;
  }
  ...
  if (identity && (identity.generation !== generation || identity.path !== descriptor.path || descriptor.remoteObjectId !== void 0 && identity.remoteObjectId !== descriptor.remoteObjectId)) issues.push(issue("other-semantic-inconsistency", "trash identity authority is inconsistent", descriptor.path, "journal-descriptor"));
...
var TRANSITIVELY_CARRIED_MOVE_REASON2 = "ancestor-folder-move-carried-descendant";
...
function transitivelyCarriedMove(operation) {
  return operation.kind === "noop" && Boolean(operation.fromPath) && Boolean(operation.toPath) && Boolean(operation.remoteObjectId) && Boolean(operation.contentVersion) && operation.reasons.some((reason) => reason.code === TRANSITIVELY_CARRIED_MOVE_REASON2);
}
...
var StateCommitCoordinator = class {
  ...
  async commitVerifiedSuccess(operation, receipt, expectedStateRevision) {
    ...
    if (transitivelyCarriedMove(operation)) {
      const oldBase = loaded.state.base.filter((entry2) => entry2.path === operation.fromPath);
      ...
      if (oldBase.length !== 1 || oldBase[0]?.remoteObjectId !== operation.remoteObjectId || byPath.length !== 1 || byId.length !== 1 || byPath[0]?.remoteObjectId !== operation.remoteObjectId || byId[0]?.path !== operation.fromPath) {
  ...
  async recordPhysicalResult(operationId, effectId, physical) {
    ...
    if (physical.status === "verified-not-applied") {
      ...
      if (!siblingProgressed) {
        ...
        const saved2 = await this.save(retired, loaded.state);
        if (saved2.status !== "persisted") return saved2;
        return { status: "verified-not-applied", reason: physical.reason, authority: saved2.authority };
      }
    }
...
var REVIEWED_FIRST_SYNC_RESOLUTION_REASONS = /* @__PURE__ */ new Set([
  "user-keep-local",
  "user-keep-remote",
  "user-manual-resolution"
]);
function reviewedFirstSyncResolutionShape(operation) {
  ...
  const remoteObject = operation.preconditions.find((precondition) => precondition.kind === "remote-object" && precondition.remoteObjectId === expectedRemoteObjectId);
  if (!localPresent || !remotePresent || !localContent?.expected.hash || localContent.expected.sizeBytes === void 0 || !remoteContent?.expected.hash || remoteContent.expected.sizeBytes === void 0 || !remoteObject || !expectedRemoteObjectId) return false;
...
function reviewedFirstSyncResolutionMayBootstrapAuthority(operation, authority, canonical2) {
  if (!reviewedFirstSyncResolutionShape(operation)) return false;
  const expectedRemoteObjectId = operation.remoteObjectId ?? operation.contentVersion?.remoteObjectId;
  if (!expectedRemoteObjectId) return false;
  if (canonical2.base.some((entry2) => entry2.path === operation.path)) return false;
  if (canonical2.remoteMappings.some((mapping) => mapping.path === operation.path || mapping.remoteObjectId === expectedRemoteObjectId)) return false;
  const pathAuthority = authority.pathConvergence.find((entry2) => entry2.path === operation.path)?.state;
...
function resolveAuthorityCompleteOperation(operation, authority, trustedRemoteMappings = [], reviewedFirstSyncResolutionBootstrap = false) {
  const preconditions = [];
...
var REVIEWED_FIRST_SYNC_RESOLUTION_REASONS2 = /* @__PURE__ */ new Set(["user-keep-local", "user-keep-remote", "user-manual-resolution"]);
...
function reviewedFirstSyncResolutionCandidate(operation) {
  return (operation.kind === "upload-update" || operation.kind === "download-update") && !operation.preconditions.some((value2) => value2.kind === "base-authority") && operation.reasons.some((reason) => REVIEWED_FIRST_SYNC_RESOLUTION_REASONS2.has(reason.code)) && operation.reasons.some((reason) => reason.code === "reviewed-first-sync-resolution");
}
function reviewedFirstSyncRecoveryIntentMatches(operation, authority) {
  const intent = authority.operationIntents.find((value2) => value2.operationId === operation.operationId);
  ...
  if (operation.kind === "upload-update") {
    return descriptor.kind === "remote-file" && descriptor.mutationKind === "update" && descriptor.targetPath === operation.path && descriptor.remoteMutation.kind === "existing-file-content-update" && descriptor.remoteMutation.remoteObjectId === expectedRemoteObjectId && descriptor.remoteMutation.identityAuthority.generation === authority.semanticGeneration && descriptor.remoteMutation.identityAuthority.path === operation.path && descriptor.remoteMutation.identityAuthority.remoteObjectId === expectedRemoteObjectId && descriptor.intendedContent.hash === expectedHash;
  }
  return descriptor.kind === "local-file" && descriptor.mutationKind === "replace" && descriptor.targetPath === operation.path && descriptor.intendedContent.hash === expectedHash;
}
async function reviewedFirstSyncResolutionEvidenceCurrent(operation, authority, identityState, legacy, managedRemote) {
  if (!reviewedFirstSyncResolutionCandidate(operation)) return false;
  const expectedRemoteObjectId = operation.remoteObjectId ?? operation.contentVersion?.remoteObjectId;
  if (!expectedRemoteObjectId) return false;
  if (identityState.state.base.some((entry2) => entry2.path === operation.path)) return false;
  if (identityState.state.remoteMappings.some((mapping) => mapping.path === operation.path || mapping.remoteObjectId === expectedRemoteObjectId)) return false;
  const currentPath = authority.pathConvergence.find((value2) => value2.path === operation.path)?.state;
  if (currentPath?.status === "converged" && currentPath.generation === authority.semanticGeneration) return false;
  const localPresent = operation.preconditions.find((value2) => value2.kind === "path-observation" && value2.side === "local" && value2.path === operation.path && value2.expected === "present");
  const remotePresent = operation.preconditions.find((value2) => value2.kind === "path-observation" && value2.side === "remote" && value2.path === operation.path && value2.expected === "present");
  const localContent = operation.preconditions.find((value2) => value2.kind === "content-evidence" && value2.side === "local" && value2.path === operation.path)?.expected;
  const remoteContent = operation.preconditions.find((value2) => value2.kind === "content-evidence" && value2.side === "remote" && value2.path === operation.path)?.expected;
  const remoteObject = operation.preconditions.find((value2) => value2.kind === "remote-object" && value2.remoteObjectId === expectedRemoteObjectId);
  if (!localPresent || !remotePresent || !localContent?.hash || localContent.sizeBytes === void 0 || !remoteContent?.hash || remoteContent.sizeBytes === void 0 || !remoteObject) return false;
  if (operation.kind === "upload-update") {
    const proof = identityAuthority(operation);
    if (!proof || proof.status !== "unique" || proof.generation !== authority.semanticGeneration || proof.path !== operation.path || proof.remoteObjectId !== expectedRemoteObjectId) return false;
    if (!localPresent.observationToken || !operation.preconditions.some((value2) => value2.kind === "file-stable" && value2.path === operation.path) || !remoteObject.expectedRevision) return false;
  }
  const listing = await internal(legacy).drive.listForReconciliation(managedRemote.rootId);
  if (!listing.ok || listing.value.completeness.status !== "complete") return false;
  const matches = listing.value.entries.filter((entry2) => !entry2.trashed && entry2.path === operation.path);
  ...
  if (remoteObject.expectedRevision !== void 0 && currentRemote.content.revision !== remoteObject.expectedRevision) return false;
  return true;
}
...
function createAuthoritativeProductExecutor(legacy, authorityStore, identityStateStore, stateContext, managedRemote, explicitDependencies) {
  ...
  async function validateExact(operation) {
    ...
    const bootstrapCurrent = bootstrapRecovery || (bootstrapCandidate ? await reviewedFirstSyncResolutionEvidenceCurrent(operation, authorityLoad.state, identityLoad, legacy, managedRemote) : false);
...
var TRANSITIVELY_CARRIED_MOVE_REASON3 = "ancestor-folder-move-carried-descendant";
function transitivelyCarriedMove2(operation) {
  return operation.kind === "noop" && Boolean(operation.fromPath) && Boolean(operation.toPath) && Boolean(operation.remoteObjectId) && Boolean(operation.contentVersion) && operation.reasons.some((reason) => reason.code === TRANSITIVELY_CARRIED_MOVE_REASON3);
}
function evidenceEqual2(actual, expected, entityKind2) {
  if (entityKind2 === "folder") return true;
  if (!actual || !expected) return false;
  if (expected.hash && actual.hash !== expected.hash) return false;
  if (expected.sizeBytes !== void 0 && actual.sizeBytes !== expected.sizeBytes) return false;
  if (expected.revision !== void 0 && actual.revision !== void 0 && actual.revision !== expected.revision) return false;
...
function createAuthoritativeProductExecutor2(legacy, authorityStore, identityStateStore, stateContext, managedRemote, explicitDependencies) {
  ...
  async function validateTransitivelyCarriedMove(operation) {
    const version = operation.contentVersion;
    ...
    const oldBase = loaded.state.base.filter((entry2) => entry2.path === fromPath);
    ...
    if (oldBase.length !== 1 || oldBase[0]?.remoteObjectId !== remoteObjectId || oldBase[0]?.entityKind !== version.entityKind || !evidenceEqual2(oldBase[0]?.content, version.content, version.entityKind) || byPath.length !== 1 || byId.length !== 1 || byPath[0]?.remoteObjectId !== remoteObjectId || byId[0]?.path !== fromPath) {
  ...
  async function validatePreconditions(operation) {
    ...
    if (transitivelyCarriedMove2(operation)) return validateTransitivelyCarriedMove(operation);
    return base.validatePreconditions(operation);
  }
  async function execute(operation) {
    const existing = await outstandingIntent(authorityStore, operation);
    if (existing.status === "recovery-required") return { status: "recovery-required", reason: existing.reason };
    if (existing.status === "none" && transitivelyCarriedMove2(operation)) {
...
var ProductControllerBase = class {
  ...
  reviewedFirstSyncConflictOrigins = /* @__PURE__ */ new Map();
  planned;
  runEvidence;
  ...
  async refreshConflicts(plan, assembly) {
    ...
    const freshFirstSyncOrigins = /* @__PURE__ */ new Map();
    ...
    for (const [key, value2] of freshFirstSyncOrigins) this.reviewedFirstSyncConflictOrigins.set(key, value2);
  }
  ...
  async resolutionOperations(_id, assessment, resolution, reviewedFirstSyncResolution) {
    ...
    if (!remoteId) return [];
    const reviewedFirstSyncReason = reviewedFirstSyncResolution ? [{ code: "reviewed-first-sync-resolution", summary: "Resolution originated from a reviewed non-reconstruction uninitialized first-sync plan." }] : [];
...
async function reconstructionMayBypassDurableRecovery(assembly, options) {
...
var ProductController = class extends ProductControllerBase {
  ...
  previewVerifyReconcile() {
    if (this.disposing) return Promise.resolve(void 0);
  ...
  requestPreviewAction(action, diagnosticRunId) {
  ...
  resolveWithCurrentLocal(id) {
    if (this.disposing) return Promise.resolve({ status: "rejected", reason: "synchronization runtime is stopping" });
    return this.track(super.resolveWithCurrentLocal(id));
  }
  async beginRuntimeDisposal() {
    if (!this.disposing) {
      this.disposing = true;
  ...
  track(promise) {
    ...
    void promise.finally(() => this.inFlight.delete(promise)).catch(() => void 0);
    return promise;
  }
};
