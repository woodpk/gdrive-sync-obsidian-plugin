...
- implementation SHA: `70ee18890603f478dfe19b9926d712b25376e84f`
...
- Existing unrelated user exclusions are preserved; changing the configured directory removes the prior managed entry only after relocation succeeds. On relocation failure, both paths remain fail-safe excluded.
...
- Replacement uses same-directory staging and backup files with best-effort rollback. Existing default stage/backup exclusion rules and internal mutation suppression prevent recursive local-change synchronization.
...
Focused mixed-plan/stability suite: **26/26 PASS**. It proves one-time stale retry and stable upload without a CSV row; exhausted instability with an independent safe commit; no cross-path contamination; true listing incompleteness; automatic creation/header; deletion and parent-deletion recreation; configured relocation; exact visible and internal exclusions; preserved user exclusions; current/history transitions; CSV escaping; restart persistence; legacy migration; and recovery after a failed CSV replacement.

Local Windows verification:

- `npm run typecheck`: PASS
- full suite: **347/349**, qualified PASS
...
GitHub/Linux **Phase 6 Alpha Diagnostic Verification** run [33266594533](https://github.com/woodpk/gdrive-sync-obsidian-plugin/actions/runs/33266594533) passed **349/349**, typecheck, complete repository check, build, diff check, and all five package verifiers. It reproduced the exact artifact below.

The separate Azure Static Web Apps preview run [33266594526](https://github.com/woodpk/gdrive-sync-obsidian-plugin/actions/runs/33266594526) failed only because the Static Web App has the maximum number of staging environments. No Azure/OAuth/workflow change was made; this is not a plugin source or verification failure.

Adversarial inspection additionally confirmed bounded retry/no starvation, no competing automatic lifecycle, exact path-local failure classification, serialized settings/write relocation, duplicate-free managed exclusions, preserved user exclusions, deletion/rename recovery, interrupted-write rollback, resolved/current truth, exact remote exclusion, and portable-configuration isolation.
...
- `main.js`: `395507` bytes
- `main.js` SHA-256: `1d0c5dd52a21cbca98584ca2a5c4ab65aef2fd1b055ab80776dd4e4bf9564496`
