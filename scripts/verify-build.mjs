...
// Temporary diagnostic for the first CI pass only: the workflow can bootstrap a
// missing npm lockfile, but PR runs cannot persist it. The next repair commit
// will replace this diagnostic with the generated lockfile and remove this block.
if (process.env.GITHUB_ACTIONS === "true" && existsSync("package-lock.json")) {
  console.log(`BOOTSTRAPPED_PACKAGE_LOCK_BASE64=${readFileSync("package-lock.json").toString("base64")}`);
}
