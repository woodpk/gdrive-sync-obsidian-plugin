...
function hash(value: string): ContentEvidence { return { hash: id<"ContentHash">(`hash:${value}`), sizeBytes: value.length }; }
...
async function conflictHarness(onTrustedBaselineEstablished?: () => Promise<void>) {
  ...
  const local = {
    ...
    readFile: async () => ({ content: source(localText), evidence: localEvidence, stability: "stable" as const }),
  ...
  await store.saveTrusted({ ...initial, base: [{ path, entityKind: "file", localExisted: true, remoteExisted: true, content: baseEvidence, remoteObjectId: remoteId }], remoteMappings: [{ path, entityKind: "file", remoteObjectId: remoteId }] });
