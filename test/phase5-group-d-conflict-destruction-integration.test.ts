...
class Boundary{
  ...
  private preservedRemotePredecessors=new Map<string,{path:VaultPath;file:F}>();
  ...
  readonly d={
    authenticationState:async()=>({status:"authenticated" as const}),validateManagedRoot:async()=>({ok:true as const,value:{status:"valid" as const,identity:this.identity}}),getStartCursor:async()=>({ok:true as const,value:id<"ChangeCursor">(`c:${++this.rev}`)}),listForReconciliation:async()=>({ok:true as const,value:{entries:[...[...this.remote].map(([p,f])=>({path:vp(p),entityKind:"file" as const,remoteObjectId:f.rid!,content:f.evidence,trashed:false})),...[...this.preservedRemotePredecessors.values()].map(({path,file:f})=>({path,entityKind:"file" as const,remoteObjectId:f.rid!,content:f.evidence,trashed:false}))],completeness:{status:"complete" as const}}}),readChanges:async()=>({ok:true as const,value:{changes:[],nextCursor:id<"ChangeCursor">(`c:${++this.rev}`),completeness:{status:"complete" as const}}}),
    ...
    download:async(r:RemoteObjectId)=>{const current=[...this.remote.entries()].find(([,f])=>f.rid===r)?.[1],preserved=[...this.preservedRemotePredecessors.values()].find(({file})=>file.rid===r)?.file,f=current??preserved;if(!f)return{ok:false as const,signal:{kind:"not-found" as const,remoteObjectId:r}};return{ok:true as const,value:{remoteObjectId:r,content:src(f.text),evidence:f.evidence}};},
  ...
  readonly reliableRemoteMutationPort:ReliableRemoteMutationPort={
    ...
    updateExisting:async(identity,content)=>{
      ...
      this.preservedRemotePredecessors.set(String(predecessor.rid),{path:identity.path,file:predecessor});
