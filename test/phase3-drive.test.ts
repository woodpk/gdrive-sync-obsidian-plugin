...
test("managed-root validation detects identity and protocol mismatch", async()=>{
  let mode="identity";
  const a=adapter(async url=>{ if(url.includes("/about")) return ok({user:{permissionId:"acct"}}); if(url.includes("/files/root?")) return ok(root(mode==="identity"?"other":"vault-1",mode==="protocol"?"2":"1")); if(isContentRootQuery(url)) return ok({files:[content()]}); throw new Error(url); });
...
test("partial reconciliation listing never masquerades as complete", async()=>{
  let childPage=0;
  const a=adapter(async url=>{ if(url.includes("/about")) return ok({user:{permissionId:"acct"}}); if(url.includes("/files/root?")) return ok(root()); if(isContentRootQuery(url)) return ok({files:[content()]}); if(url.includes("q=%27content%27")||url.includes("q='content'")){ childPage++; if(childPage===1) return ok({files:[{id:"f1",name:"a.md",mimeType:"text/plain",parents:["content"],trashed:false,size:"1",version:"1"}],nextPageToken:"next"}); return {ok:false,signal:{kind:"transient-failure",detail:"network"}}; } throw new Error(url); });
...
test("move preserves Drive id and normal deletion uses trash PATCH", async()=>{
  ...
  const a=adapter(async (url,init)=>{ seen.push({url,method:init?.method,body:typeof init?.body==="string"?init.body:undefined}); if(url.includes("/about")) return ok({user:{permissionId:"acct"}}); if(url.includes("/files/file1?")){ if(init?.method==="PATCH") return ok({id:"file1",name:"new.md",mimeType:"text/plain",parents:["newp"],size:"3",version:"2"}); return ok({id:"file1",name:"old.md",mimeType:"text/plain",parents:["content"],size:"3",version:"1"}); } if(url.includes("/files/content?")) return ok(content()); if(url.includes("/files/root?")) return ok(root()); if(isContentRootQuery(url)) return ok({files:[content()]}); if(url.includes("name%3D%27new%27")||url.includes("name%3D'new'")) return ok({files:[{id:"newp",name:"new",mimeType:"application/vnd.google-apps.folder",parents:["content"],trashed:false}]}); if(url.includes("/files/file2?")) return ok({id:"file2",trashed:true}); throw new Error(url); });
